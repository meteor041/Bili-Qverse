...
(function () {
    ...
    useEffect(() => {
      ...
      async function load() {
        ...
        try {
          ...
          const [rankingRes, statsRes] = await Promise.all([fetch(`/api/ranking?${params.toString()}`), fetch('/api/stats/realtime')]);
  ...
  function GlobalCounter({
    ...
    }, "\u4ECA\u65E5\u5DF2\u91C7\u96C6\u95EE\u53F7\u5F39\u5E55\u603B\u91CF"), React.createElement("span", {
      style: {
        fontSize: 20,
        fontWeight: 900,
        color: 'var(--qc)',
  ...
  function TrendPanel({
    ...
    }, ['#离谱', '#看不懂', '#真的假的', '#神奇操作', '#迷惑现场', '#问号爆炸', '#神仙打法', '#玄学'].map(t => React.createElement("span", {
      key: t,
      style: {
        fontSize: 11,
        padding: '4px 10px',
  ...
  function CompactRow({
    ...
    return React.createElement("div", {
      ...
      style: {
        display: 'grid',
        gridTemplateColumns: '36px 56px 1fr auto auto auto',
        alignItems: 'center',
        gap: 12,
        padding: '10px 14px',
      ...
      style: {
        ...
        flexShrink: 0,
        position: 'relative',
        background: `radial-gradient(circle at 30% 40%,${PAL[1]},${PAL[0]})`,
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'center'
      }
    ...
    }, '？'.repeat(Math.min(3, paletteIndex % 3 + 1))), React.createElement("div", {
      style: {
        position: 'absolute',
        bottom: 2,
        right: 3,
    ...
    }, vid.duration)), React.createElement("div", {
      style: {
        minWidth: 0
      }
    }, React.createElement("div", {
  ...
  function ConfusionCalc() {
    ...
    const scanMessages = ['正在分析标题语义…', '检测问号密度…', '扫描弹幕特征词…', '计算迷惑系数…', '比对历史数据库…', '运行疑问指数模型…', '生成最终结果…'];
    function calculate() {
      ...
      const iv = setInterval(() => {
        if (i < scanMessages.length) {
          ...
          clearInterval(iv);
          fetch(`/api/tools/estimate?text=${encodeURIComponent(input)}`).then(res => res.ok ? res.json() : null).then(data => {
            const fallback = () => {
              const qCount = (input.match(/[？?]/g) || []).length;
              const keywords = ['离谱', '不懂', '看不懂', '什么', '怎么', '为什么', '神', '诶', '诶诶', '啊', '吗', '吧'].filter(k => input.includes(k)).length;
              return Math.min(100, Math.round(20 + qCount * 12 + keywords * 8 + (input.length > 10 ? 15 : 0) + Math.random() * 20));
            };
            const raw = data?.score || fallback();
            setScore(raw);
            setStage('result');
            if (raw >= 80) {
              setQExplosion(true);
            ...
            const qCount = (input.match(/[？?]/g) || []).length;
            const keywords = ['离谱', '不懂', '看不懂', '什么', '怎么', '为什么', '神', '诶', '诶诶', '啊', '吗', '吧'].filter(k => input.includes(k)).length;
            const raw = Math.min(100, Math.round(20 + qCount * 12 + keywords * 8 + (input.length > 10 ? 15 : 0) + Math.random() * 20));
            setScore(raw);
            setStage('result');
            if (raw >= 80) {
              setQExplosion(true);
    ...
    return React.createElement("div", {
    ...
    }, "\u8F93\u5165\u89C6\u9891\u6807\u9898\uFF0CAI \u5E2E\u4F60\u8BA1\u7B97\u6709\u591A\u79BB\u8C31")), React.createElement("div", {
      style: {
        background: 'var(--card)',
        border: '1px solid var(--border)',
        borderRadius: 20,
      ...
      placeholder: "\u8F93\u5165\u89C6\u9891\u6807\u9898\uFF0C\u4F8B\u5982\uFF1A\u8FD9\u4E2A\u4EBA\u5230\u5E95\u5728\u5E72\u561B\uFF1F\uFF1F\uFF1F",
      style: {
        width: '100%',
        background: 'var(--card2)',
        border: '1px solid var(--border)',
    ...
    }, stage === 'scanning' ? '分析中…' : '🧮 开始计算')), stage === 'scanning' && React.createElement("div", {
      style: {
        background: 'var(--card)',
        border: '1px solid rgba(0,229,255,.2)',
        borderRadius: 16,
    ...
    }), "AI \u5206\u6790\u4E2D"), logs.map((l, i) => React.createElement("div", {
      key: i,
      style: {
        fontSize: 12,
        color: 'var(--txt2)',
    ...
    }, "\u300C", input, "\u300D", React.createElement("br", null), "\u7684\u7591\u95EE\u6307\u6570\u4E3A ", React.createElement("span", {
      style: {
        color: getColor(score),
        fontWeight: 700
      }
  ...
  function DetailPage({
    ...
    const detailVid = {
      ...
      tags: Array.isArray(fallbackVid.tags) ? fallbackVid.tags : [],
      src: fallbackVid.src || {
        d: 100,
        c: 0,
        t: 0
    ...
    }, l))))), tab === 'stats' && React.createElement("div", null, React.createElement("div", {
      style: {
        fontSize: 14,
        fontWeight: 700,
        marginBottom: 14
